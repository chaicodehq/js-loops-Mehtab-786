...
export function cricketScoreboard(balls) {
  ...
  let obj = { totalRuns: 0, totalBalls: 0, wickets: 0, fours: 0, sixes: 0 };
  for (let i = 0; i < balls.length; i++) {

    let ball = balls[i];
    if (obj['wickets'] >= 10) break;

    obj['totalBalls'] = obj['totalBalls'] + 1;

    if (ball >= 1 && ball <= 6) {
      if (ball === 4) obj['fours'] = obj['fours'] + 1;
      else if (ball === 6) obj['sixes'] = obj['sixes'] + 1;

      obj['totalRuns'] = obj['totalRuns'] + ball;

    } else if (ball === -1) {
      obj['wickets'] = obj['wickets'] + 1;
    } else if (ball === 0) continue;
